...
export default function SpiritualAssessment({ onComplete, onBack, userRole }: SpiritualAssessmentProps) {
  ...
  // Create mixed questions - one from each category in rotation to prevent gaming
  const createMixedAssessment = () => {
    const categories = [
      {
        name: "Leadership",
        questions: [
      ...
      },
      {
        name: "Hospitality",
        questions: [
          "I love welcoming people into my home.",
          "I enjoy preparing meals and creating comfortable spaces for others.",
          "I find it stressful to have people over to my house.", // INVERSE
          "I am energized by hosting gatherings and bringing people together.",
          "I can make people feel valued and comfortable when they visit.",
          "I prefer to meet people in public places rather than my home.", // INVERSE
          "I enjoy decorating and preparing spaces to be inviting.",
          "I feel blessed when I can provide a place for fellowship.",
          "I find it inconvenient when people drop by unexpectedly.", // INVERSE
          "I believe that sharing meals together is an important ministry."
        ]
      },
      {
        name: "Faith",
        questions: [
          "I have a strong confidence that God will provide for my needs.",
          "I can trust God even when circumstances seem impossible.",
          ...
          "I find it hard to believe that God will come through in tough situations.", // INVERSE
          "I can see God's hand at work even when others are filled with doubt."
        ]
      },
      {
        name: "Wisdom",
    ...
    // Mix questions: take one from each category in rotation
    for (let round = 0; round < 10; round++) {
      for (let catIndex = 0; catIndex < categories.length; catIndex++) {
        mixedQuestions.push(categories[catIndex].questions[round]);
        questionCategories.push(categories[catIndex].name);
      }
    }
    
    return { questions: mixedQuestions, categories: questionCategories };
  };
  ...
  const getGiftCategory = (questionIndex: number) => {
    // Now using the mixed categories array
    return questionCategories[questionIndex] || "Unknown";
  };
