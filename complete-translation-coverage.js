...
async function addMissingTranslations() {
  ...
  const { rows: finalStats } = await pool.query(`
    SELECT 
      ...
      ROUND(AVG(verse_coverage.coverage), 2) as avg_translations_per_verse
    FROM bible_verses,
    (SELECT reference, COUNT(*) as coverage FROM bible_verses GROUP BY reference) as verse_coverage
    WHERE bible_verses.reference = verse_coverage.reference
  `);
  ...
  console.log(`   Average translations per verse: ${finalStats[0].avg_translations_per_verse}`);
