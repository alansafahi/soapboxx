...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/users/search', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const userId = req.session?.userId || (req.user as any)?.id || (req.user as any)?.claims?.sub || (req.user as any)?.id;
  ...
  app.post('/api/bible/lookup', async (req: any, res) => {
    try {
      ...
      if (!version && req.session?.userId) {
        try {
          const user = await storage.getUser(req.session?.userId || (req.user as any)?.id || (req.user as any)?.claims?.sub);
  ...
  app.get('/api/user-profiles/role', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.session?.userId || (req.user as any)?.id || (req.user as any)?.claims?.sub || (req.user as any)?.claims?.sub || (req.user as any)?.id;
  ...
  app.post('/api/auth/send-sms-verification', isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.session?.userId || (req.user as any)?.id || (req.user as any)?.claims?.sub || (req.user as any)?.claims?.sub;
      if (!userId) {
        return res.status(401).json({ message: 'Unauthorized' });
      }
    ...
    try {
      ...
      const response = await openai.chat.completions.create({
        ...
        messages: [
          {
            role: "system", 
            content: "You are a creative Sunday School activity developer. Create fun, story-specific activities that use the exact Bible characters, settings, and events mentioned. Make activities that children will remember because they connect directly to the Bible story details. Be creative with names and content - use character names in activity titles and reference specific story events in instructions."
          },
          {
            role: "user",
            content: prompt
      ...
      try {
        ...
        // Add comprehensive teacher guidance as separate instructions
        const teacherInstructions = {
          generalGuidance: {
            classroomSetup: "Arrange students in a semicircle facing the teacher. Place all materials on a central table within easy reach. Ensure adequate space for movement and interaction.",
            preparationTips: "Review all activities 30 minutes before class. Pre-cut materials and test any technology. Have backup activities ready for different group sizes.",
            timingGuidance: `For ${ageGroup} children: ${ageGroup === 'preschool' ? 'Keep activities to 5-7 minutes max with frequent movement breaks' : ageGroup === 'elementary' ? 'Activities can be 8-12 minutes with clear start/stop points' : 'Activities can be 12-18 minutes with deeper engagement'}`,
            classroomManagement: "Use positive reinforcement and name recognition. Give clear, step-by-step instructions. Provide gentle redirection for off-task behavior. Include all students by asking different types of questions.",
            safetyReminders: "Ensure clear pathways for movement. Check all materials for age appropriateness. Have hand sanitizer available. Monitor group interactions.",
            adaptations: "For large groups: divide into smaller teams. For special needs: provide visual aids and alternative participation methods. For mixed ages: offer different complexity levels."
          },
          ageSpecificGuidance: ageGroup === 'preschool' ? {
            tips: "Use simple language, repeat instructions, include sensory elements, provide immediate praise, allow for short attention spans",
            transitions: "Use songs or countdowns to signal activity changes",
            materials: "Ensure all items are safe for young children - no small parts"
          } : ageGroup === 'elementary' ? {
            tips: "Include competition elements, hands-on learning, clear rules, opportunities for questions and participation",
            transitions: "Use clear signals and expectations for movement between activities",
            materials: "Have extra supplies available as children this age may need multiple attempts"
          } : {
            tips: "Encourage peer interaction, provide leadership opportunities, connect to real-world applications",
            transitions: "Allow students to help with setup and cleanup",
            materials: "Include technology integration where appropriate"
          }
        };
        ...
        res.json({
          activities,
          teacherInstructions,
          lessonFocus: `${topic} - Story-specific activities created`,
          metadata: {
            ageGroup,
            duration: duration || 'medium',
            storyElements: `Characters: ${topic.includes('Moses') ? 'Moses, Pharaoh, Israelites' : topic.includes('David') ? 'David, Goliath, King Saul' : topic.includes('Mary') ? 'Mary, Angel Gabriel, Joseph' : 'Bible characters'}`
          }
        });
  ...
  // Get comprehensive Sunday School teaching guidelines
  app.get('/api/sunday-school/teaching-guidelines', async (req, res) => {
    try {
      const guidelines = {
        generalInstructions: {
          classroomSetup: "Arrange students in a semicircle facing the teacher. Ensure all materials are within easy reach and visual aids are at student eye level.",
          teachingTips: "Use enthusiasm and variety in your voice. Make eye contact with students. Pause frequently for questions and participation.",
          timeManagement: "Keep activities moving with clear transitions. Use timers to help students understand activity lengths.",
          behaviorManagement: "Use positive reinforcement, clear expectations, and engaging activities to maintain attention."
        },
        ageSpecificGuidelines: {
          preschool: {
            attentionSpan: "5-7 minutes per activity",
            learningStyle: "Sensory play, simple crafts, action songs, story props",
            vocabulary: "Simple, concrete language with repetition",
            classroom: "Frequent movement breaks, hands-on activities, immediate reinforcement"
          },
          elementary: {
            attentionSpan: "8-12 minutes per activity", 
            learningStyle: "Games with rules, competitive activities, group crafts, drama",
            vocabulary: "Expanding vocabulary with concrete examples",
            classroom: "Clear start/stop points, peer interaction, hands-on learning"
          },
          middle: {
            attentionSpan: "12-18 minutes per activity",
            learningStyle: "Team challenges, discussions, creative projects, technology",
            vocabulary: "Abstract concepts with personal application",
            classroom: "Peer interaction, leadership opportunities, deeper engagement"
          },
          high: {
            attentionSpan: "15-25 minutes per activity",
            learningStyle: "Leadership activities, real-world applications, ministry projects",
            vocabulary: "Advanced concepts with life application",
            classroom: "Independent thinking, mentoring roles, critical discussion"
          }
        },
        professionalStandards: {
          preparation: "Review lesson materials 24 hours in advance. Prepare all supplies and test equipment.",
          biblicalAccuracy: "Verify all scripture references and theological content for age-appropriate accuracy.",
          safetyFirst: "Always consider safety in activity planning. Have emergency procedures ready.",
          parentCommunication: "Provide take-home materials that help parents continue learning at home."
        }
      };

      res.json(guidelines);
    } catch (error) {
      res.status(500).json({ message: "Failed to fetch teaching guidelines" });
    }
  });
...
Respond in JSON format with these keys: reflectionQuestions (array), practicalApplication (string), prayer (string)`;
  ...
  app.post('/api/messages', isAuthenticated, async (req: any, res) => {
    try {
      const senderId = req.session?.userId || (req.user as any)?.id || (req.user as any)?.claims?.sub;
  ...
  app.post('/api/prayer-analytics/answered-prayer-testimony', isAuthenticated, async (req, res) => {
    try {
      const testimony = await storage.createAnsweredPrayerTestimony({
        ...req.body,
        userId: req.session?.userId || (req.user as any)?.id || (req.user as any)?.claims?.sub,
      });
      res.json(testimony);
    } catch (error) {
  ...
  app.post('/api/prayer-analytics/react-answered-prayer', isAuthenticated, async (req, res) => {
    try {
      ...
      await storage.reactToAnsweredPrayer(testimonyId, req.session?.userId || (req.user as any)?.id || (req.user as any)?.claims?.sub, reactionType);
      res.json({ success: true });
    } catch (error) {
  ...
  app.get('/api/prayer-analytics/trends', isAuthenticated, async (req, res) => {
    try {
      ...
      const userChurch = await storage.getUserChurch(req.session?.userId || (req.user as any)?.id || (req.user as any)?.claims?.sub);
  ...
  app.post('/api/prayer-analytics/track-activity', isAuthenticated, async (req, res) => {
    try {
      ...
      await storage.updateUserProgress(req.session?.userId || (req.user as any)?.id || (req.user as any)?.claims?.sub, activityType, entityId);
      res.json({ success: true });
    } catch (error) {
  ...
  app.post('/api/admin/initialize-badges', isAuthenticated, async (req, res) => {
    try {
      // Check if user has admin permissions
      const userRole = await storage.getUserRole(req.session?.userId || (req.user as any)?.id || (req.user as any)?.claims?.sub);
...
Return JSON with this exact structure:
{
  ...
  app.post('/api/churches', isAuthenticated, upload.single('logo'), async (req: any, res) => {
    try {
      ...
      await storage.joinChurch(req.session?.userId || (req.user as any)?.id || (req.user as any)?.claims?.sub, newChurch.id);
      await storage.updateUserChurchRole(req.session?.userId || (req.user as any)?.id || (req.user as any)?.claims?.sub, newChurch.id, 'church_admin');
      ...
      try {
        await storage.initializeChurchFeatures(newChurch.id, sizeMapping[churchSize as keyof typeof sizeMapping] || 'small', req.session?.userId || (req.user as any)?.id || (req.user as any)?.claims?.sub);
      } catch (error) {
  ...
  app.get('/api/users/created-churches', isAuthenticated, async (req: any, res) => {
    try {
      const userId = (req.user as any)?.claims?.sub || req.session?.userId;
  ...
  app.put('/api/users/profile', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const updateData = {
        ...
        bibleTranslationPreference: req.body.bibleTranslationPreference,
        smallGroup: req.body.smallGroup,
        smallGroupParticipation: req.body.smallGroupParticipation,
  ...
  app.get("/api/feed", isAuthenticated, async (req: any, res) => {
    try {
      ...
      // Feed posts retrieved with verification data
...
Please provide suggestions for the missing or incomplete sections.`
  ...
  app.post('/api/upload/image', isAuthenticated, upload.single('file'), async (req: any, res) => {
    try {
      const userId = req.session?.userId || (req.user as any)?.claims?.sub || (req.user as any)?.id;
  ...
  app.post('/api/volunteers', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const [volunteer] = await db
        ...
        .values({
          userId: (req.user as any)?.id || '1', // Fallback for test data  
  ...
  app.get("/api/milestones", isAuthenticated, async (req: RequestWithSession, res) => {
    try {
      const progress = await milestoneService.getUserMilestoneProgress(req.session?.userId || (req.user as any)?.id || (req.user as any)?.claims?.sub!);
  ...
  app.get("/api/achievements", isAuthenticated, async (req: RequestWithSession, res) => {
    try {
      const achievements = await storage.getUserAchievements(req.session?.userId || (req.user as any)?.id || (req.user as any)?.claims?.sub!);
      res.json({ achievements });
    } catch (error) {
  ...
  app.post("/api/milestones/check", isAuthenticated, async (req: RequestWithSession, res) => {
    try {
      ...
      await milestoneService.checkMilestones(req.session?.userId || (req.user as any)?.id || (req.user as any)?.claims?.sub!, activityType, data);
  ...
  app.post("/api/push/subscribe", isAuthenticated, async (req: RequestWithSession, res) => {
    try {
      ...
      const result = await storage.savePushSubscription(req.session?.userId || (req.user as any)?.id || (req.user as any)?.claims?.sub!, subscription);
  ...
  app.post("/api/push/unsubscribe", isAuthenticated, async (req: RequestWithSession, res) => {
    try {
      ...
      const subscriptions = await storage.getUserPushSubscriptions(req.session?.userId || (req.user as any)?.id || (req.user as any)?.claims?.sub!);
  ...
  app.post("/api/push/test", isAuthenticated, async (req: RequestWithSession, res) => {
    try {
      const { webPushService } = require('./web-push-service');
      const success = await webPushService.sendNotificationToUser(req.session?.userId || (req.user as any)?.id || (req.user as any)?.claims?.sub!, {
  ...
  app.get("/api/notification-preferences", isAuthenticated, async (req: RequestWithSession, res) => {
    try {
      const preferences = await storage.getNotificationPreferences(req.session?.userId || (req.user as any)?.id || (req.user as any)?.claims?.sub!);
  ...
  app.put("/api/notification-preferences", isAuthenticated, async (req: RequestWithSession, res) => {
    try {
      const updates = req.body;
      await storage.updateNotificationPreferences(req.session?.userId || (req.user as any)?.id || (req.user as any)?.claims?.sub!, updates);
      res.json({ success: true });
    } catch (error) {
  ...
  app.get("/api/weekly-checkin/status", isAuthenticated, async (req: RequestWithSession, res) => {
    try {
      ...
      const hasCompleted = await weeklyCheckinService.hasCompletedThisWeek(req.session?.userId || (req.user as any)?.id || (req.user as any)?.claims?.sub!);
  ...
  app.get("/api/weekly-checkin/stats", isAuthenticated, async (req: RequestWithSession, res) => {
    try {
      const { weeklyCheckinService } = await import('./weekly-checkin-service');
      const stats = await weeklyCheckinService.getUserCheckinStats(req.session?.userId || (req.user as any)?.id || (req.user as any)?.claims?.sub!);
      res.json(stats);
    } catch (error) {
  ...
  app.get("/api/weekly-checkin/history", isAuthenticated, async (req: RequestWithSession, res) => {
    try {
      const { weeklyCheckinService } = await import('./weekly-checkin-service');
      const history = await weeklyCheckinService.getUserCheckinHistory(req.session?.userId || (req.user as any)?.id || (req.user as any)?.claims?.sub!, 12);
      res.json(history);
    } catch (error) {
