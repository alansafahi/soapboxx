...
async function saveVerse(reference: string, translation: string, apiResult: ApiVerseResult, popularityRank: number): Promise<boolean> {
  try {
    ...
    console.log(`✓ Saved: ${reference} (${translation}) - Rank ${popularityRank}`);
    return true;
  } catch (error) {
  ...
  for (let i = 0; i < popularVerses.length; i++) {
    ...
    try {
      ...
      if (await verseExists(reference, translation)) {
        console.log(`Already exists: ${reference} (${translation})`);
        result.duplicates++;
        continue;
      }
      ...
      if (result.apiCallsMade >= maxDailyRequests) {
        console.log(`\nDaily rate limit reached (${maxDailyRequests} requests)`);
      ...
      // Progress indicator
      if (i % 25 === 0) {
        console.log(`Progress: ${i + 1}/${popularVerses.length} verses, ${result.apiCallsMade}/${maxDailyRequests} API calls today`);
      }
...
export async function import1000PopularVerses(maxDailyRequests: number = 800): Promise<void> {
  ...
  console.log(`Daily rate limit: ${maxDailyRequests} API requests`);
  ...
  for (const translation of Object.keys(BIBLE_VERSIONS)) {
    try {
      ...
      if (result.totalProcessed < popularVerses.length) {
        console.log(`\nStopping import for today due to rate limits`);
  ...
  console.log(`API calls made: ${overallResults.totalApiCalls}`);
  ...
  if (finalProgress.completedCount < totalPossible) {
    ...
    console.log(`All 1,000 verses imported successfully across all translations!`);
  }
}
