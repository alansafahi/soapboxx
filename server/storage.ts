...
export class DatabaseStorage implements IStorage {
    ...
    const discussionsQuery = db
      ...
      .where(
        and(
          ...
          includeFlagged ? undefined : eq(discussions.isPublic, true), // Include all when flagged, otherwise only public
          includeFlagged ? undefined : isNull(contentReports.id), // Include flagged when requested
    ...
    const soapQuery = db
      ...
      .where(
        and(
          ...
          includeFlagged ? undefined : eq(soapEntries.isPublic, true), // Include all when flagged
          includeFlagged ? undefined : isNull(contentReports.id), // Include flagged when requested
          ...
          const userPrayerReaction = await db
            ...
            .where(
              and(
                ...
                eq(reactions.targetType, post.type === 'soap_reflection' ? 'soap' : 'discussion'),
                eq(reactions.targetId, post.id),
                eq(reactions.reactionType, 'prayer')
              )
            );
  ...
  async getSimpleDiscussions(): Promise<any[]> {
    const result = await db
      .select()
      .from(discussions)
      .where(eq(discussions.isPublic, true))
      .limit(10);
    return result;
  }
