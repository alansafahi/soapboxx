  ...
  async addSoapReaction(soapId: number, userId: string, reactionType: string, emoji: string): Promise<void> {
    try {
      ...
      if (existingReaction.rows && existingReaction.rows.length > 0) {
        await db.execute(sql`
          DELETE FROM reactions WHERE id = ${existingReaction.rows[0].id}
        `);
        return;
      }

      // Step 2: Check if user has any other reaction for this SOAP entry
      const otherReaction = await db.execute(sql`
        SELECT id FROM reactions 
        WHERE target_id = ${soapId} 
        AND user_id = ${userId} 
        AND target_type = 'soap_entry'
      ...
      // If other reaction exists, update it
      if (otherReaction.rows && otherReaction.rows.length > 0) {
        await db.execute(sql`
          UPDATE reactions 
          SET reaction_type = ${reactionType}, emoji = ${emoji}
          WHERE id = ${otherReaction.rows[0].id}
        `);
      } else {
        // Step 3: Create new reaction
        await db.execute(sql`
          INSERT INTO reactions (target_id, user_id, target_type, reaction_type, emoji, intensity, created_at)
          VALUES (${soapId}, ${userId}, 'soap_entry', ${reactionType}, ${emoji}, 1, NOW())
        `);
      }
    } catch (error) {
      console.error('Error in addSoapReaction:', error);
      throw error;
    }
  }

  // Get SOAP reaction counts by type for a specific SOAP entry
  async getSoapReactionCounts(soapId: number): Promise<{ [reactionType: string]: number }> {
    try {
      const result = await db.execute(sql`
      ...
      const counts: { [reactionType: string]: number } = {};
      if (result.rows) {
        for (const row of result.rows) {
          counts[row.reaction_type] = parseInt(row.count as string);
        }
      }
      return counts;
    } catch (error) {
      console.error('Error getting SOAP reaction counts:', error);
      return {};
    }
  }

  // Get user's reaction for a specific SOAP entry
  async getUserSoapReaction(soapId: number, userId: string): Promise<{ reactionType: string; emoji: string } | null> {
    try {
      const result = await db.execute(sql`
      ...
      if (result.rows && result.rows.length > 0) {
        return {
          reactionType: result.rows[0].reaction_type as string,
          emoji: result.rows[0].emoji as string
        };
      }
      return null;
    } catch (error) {
      console.error('Error getting user SOAP reaction:', error);
      return null;
    }
  }
  ...
  async inviteStaffMember(data: {
    ...
    try {
      ...
      if (existingUser) {
        ...
        await pool.query(`
          INSERT INTO user_churches (user_id, church_id, role, title, department, assigned_by, assigned_at, joined_at, is_active)
        ...
        await pool.query(`
          INSERT INTO user_churches (user_id, church_id, role, title, department, assigned_by, assigned_at, is_active)
